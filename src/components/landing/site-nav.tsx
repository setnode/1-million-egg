...
export function SiteNav() {
  ...
  return (
    <header
      className="fixed inset-x-0 top-0 z-50 border-b border-border/40 bg-background/80 backdrop-blur-lg transition-all duration-300"
    >
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        <Link href="/" aria-label="1 Million Egg home">
          <Logo />
        </Link>
      ...
      {open && (
        <div className="border-t border-border/70 bg-background/95 px-5 pb-6 pt-2 backdrop-blur-xl md:hidden">
          <div className="flex flex-col">
            ...
            <Button onClick={openConnectModal} className="mt-3 w-full rounded-full shadow-premium">
              Play Now
            </Button>
          </div>
        </div>
